...
app.config['MAIL_USERNAME'] = 'ibmproject'
app.config['MAIL_PASSWORD'] = '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'
app.config['MAIL_DEFAULT_SENDER'] = 'srinivaspr2000@gmail.com'
mail = Mail(app)
cors = CORS(app)
'''
...
def registration():
    ...
    if request.method=='POST':
        ...
        sql="INSERT INTO PETA_USER(USERID,EMAIL,PASSWORD,WALLET) VALUES(?,?,?,?)"
        stmt=ibm_db.prepare(conn,sql)
        ibm_db.bind_param(stmt,1,USERID)
        ibm_db.bind_param(stmt,2,email)
        ibm_db.bind_param(stmt,3,password)
        ibm_db.bind_param(stmt,4,wallet)
        ibm_db.execute(stmt)
        '''
